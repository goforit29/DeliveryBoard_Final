...
public class DeliveryboardViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenReviewed_then_UPDATE_5(@Payload Reviewed reviewed) {
        try {
            if (reviewed.isMe()) {
                // view 객체 조회
                List<Deliveryboard> deliveryboardList = deliveryboardRepository.findByRequestId(reviewed.getRequestId());
                for(Deliveryboard deliveryboard : deliveryboardList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    deliveryboard.setReview(reviewed.getReview());
                    // view 레파지 토리에 save
                    deliveryboardRepository.save(deliveryboard);
                }
            }
